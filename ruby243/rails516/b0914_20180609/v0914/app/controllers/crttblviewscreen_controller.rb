...
###  xxx_idの重複チェック
	...
	def index
		if ENV["RACK_ENV"] == "development" ###開発環境のみで動く
			...
			when /screens/
				if  rec = ActiveRecord::Base.connection.select_one("select * from r_screens where id = #{params[:jqgrid_id]}  ")
					if rec["screen_expiredate"] > Time.now
						tbl_rec = ActiveRecord::Base.connection.select_one("select * from r_blktbs where pobject_code_tbl = '#{rec["pobject_code_scr"].split("_")[1]}'  ")
						create_or_replace_view  tbl_rec["id"],tbl_rec["pobject_code_tbl"],params[:jqgrid_id]
						##Rails.cache.clear(nil)
					else
						@errmsg = "out of expiredate"
					end
  ...
  def prv_add_field tblname,frec
        ...
        subfields.each do |rec|
          ...
          if  js =~ /_id/
            ...
            when /persons_id_upd/   ##person は特殊
	          ...
	          when /prjnos_id_chil/   #特殊
	            join_rtbl = "chil_prjnos"
            else
    ...
    k = ""
	  unless ActiveRecord::Base.connection.table_exists?(join_rtbl)
           @errmsg << " view #{ join_rtbl } not exists "
           raise
