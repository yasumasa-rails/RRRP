 ...
 --- drop view pack_nditms cascade  
 create or replace view pack_nditms as select  
...
nditm.consumauto  nditm_consumauto,
  ...
  opeitm_chil.packqty opeitm_packqty_chil
 from nditms   nditm,
  r_itms  itm_nditm ,  r_persons  person_upd ,  r_opeitms  opeitm ,  r_locas  loca_fm ,  r_crrs  crr ,
  opeitms opeitm_chil
  where       nditm.itms_id_nditm = itm_nditm.id      and nditm.persons_id_upd = person_upd.id     
 and nditm.opeitms_id = opeitm.id      and nditm.locas_id_fm = loca_fm.id      and nditm.crrs_id = crr.id  
	AND opeitm_chil.itms_id =  nditm.itms_id_nditm AND 	opeitm_chil.processseq =  nditm.processseq_nditm
	AND opeitm_chil.priority = 999;
 DROP TABLE IF EXISTS sio.sio_pack_nditms;
 CREATE TABLE sio.sio_pack_nditms (
          sio_id numeric(38,0)  CONSTRAINT SIO_pack_nditms_id_pk PRIMARY KEY           ,sio_user_code numeric(38,0)
...
,opeitm_packqty_chil  numeric (38,0)
...
,opeitm_units_lttime  varchar (4) 
 ...
 CREATE INDEX sio_pack_nditms_uk1 
  ON sio.sio_pack_nditms(sio_user_code,sio_session_counter,sio_session_id,sio_Command_Response); 

 drop sequence  if exists sio.sio_pack_nditms_seq ;
 create sequence sio.sio_pack_nditms_seq ;
