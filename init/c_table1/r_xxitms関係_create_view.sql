...
,id  numeric (22,0)
 ...
 create sequence sio.sio_r_units_seq ;
  drop view if  exists r_boxes cascade ; 
...
boxe.name  boxe_name
 from boxes   boxe,
  r_persons  person_upd ,  r_units  unit_box ,  r_units  unit_outbox 
  where       boxe.persons_id_upd = person_upd.id      and boxe.units_id_box = unit_box.id      and boxe.units_id_outbox = unit_outbox.id     ;
 DROP TABLE IF EXISTS sio.sio_r_boxes;
 CREATE TABLE sio.sio_r_boxes (
          sio_id numeric(22,0)  CONSTRAINT SIO_r_boxes_id_pk PRIMARY KEY           ,sio_user_code numeric(22,0)
          ,sio_Term_id varchar(30)
          ,sio_session_id numeric(22,0)
          ,sio_Command_Response char(1)
          ,sio_session_counter numeric(22,0)
...
,boxe_id  numeric (38,0)
 ...
 drop  MATERIALIZED  view if exists r_items cascade;  
 create MATERIALIZED  view r_itms as select  
itm.model  itm_model,
itm.deth  itm_deth,
...
,itm_unit_id  numeric (22,0)
 ...
 drop  MATERIALIZED  view if exists r_opeitems cascade;  
 create MATERIALIZED  view r_opeitms as select  
...
opeitm.locas_id_opeitm   opeitm_loca_id_opeitm,
 ...
 from opeitms   opeitm,
  r_units  unit_prdpurshp ,  r_persons  person_upd ,  r_itms  itm ,  r_units  unit_case ,  r_boxes  boxe ,  r_shelfnos  shelfno ,  r_locas  loca_opeitm 
  where       opeitm.units_id_prdpurshp = unit_prdpurshp.id      and opeitm.persons_id_upd = person_upd.id      and opeitm.itms_id = itm.id      and opeitm.units_id_case = unit_case.id      and opeitm.boxes_id = boxe.id      and opeitm.shelfnos_id = shelfno.id      and opeitm.locas_id_opeitm = loca_opeitm.id     ;
...
,opeitm_unit_id_case  numeric (38,0)
 ...
 drop  MATERIALIZED  view if exists r_nditems cascade;  
 create MATERIALIZED  view r_nditms as select  
