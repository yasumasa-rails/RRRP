 ...
 alter table  shpschs  ADD COLUMN crrs_id numeric(22,0)  DEFAULT 0  not null;
...
shpsch.amt_sch  shpsch_amt_sch,
  prjno.prjno_name_chil  prjno_name_chil 
 from shpschs   shpsch,
  r_itms  itm ,  r_locas  loca_to ,  r_persons  person_upd ,  r_prjnos  prjno ,  r_chrgs  chrg ,  r_transports  transport ,  r_crrs  crr ,  r_shelfnos  shelfno_fm 
  where       shpsch.itms_id = itm.id      and shpsch.locas_id_to = loca_to.id      and shpsch.persons_id_upd = person_upd.id      and shpsch.prjnos_id = prjno.id      and shpsch.chrgs_id = chrg.id      and shpsch.transports_id = transport.id      and shpsch.crrs_id = crr.id      and shpsch.shelfnos_id_fm = shelfno_fm.id     ;
