 ...
 alter table  custdlvs  ADD COLUMN opeitms_id numeric(38,0)  DEFAULT 0  not null;

 alter table custdlvs DROP COLUMN itms_id CASCADE;
 ...
 ---- update screenfields set expiredate ='2000/01/01',remark =' 項目　itms_idが削除　2021-04-16 21:00:53 +0900' 
 ---- where  pobject_code_sfd = 'itms_id'
 alter table purdlvs DROP COLUMN replydate CASCADE;
 ...
 --- select * from pobject_code_scr,pobject_code_sfd,
							...
							---	case screenfield_hideflg when 1 then '' else '表示有' end display,
							---   case screenfield_indisp when 1 then '必須' else '' end inquire from r_screenfields 
 ---- where  pobject_code_sfd = 'replydate'
 ---- update screenfields set expiredate ='2000/01/01',remark =' 項目　replydateが削除　2021-04-16 21:00:53 +0900' 
 ---- where  pobject_code_sfd = 'replydate'
 alter table purrets  ADD COLUMN sno_puract varchar(50);
...
custdlv.id id,
  ...
  asstwh.chrg_id_asstwh  chrg_id_asstwh ,
  ...
  asstwh.loca_code_sect_asstwh  loca_code_sect_asstwh ,
  asstwh.loca_name_sect_asstwh  loca_name_sect_asstwh ,
...
,loca_code_sect_asstwh  varchar (50) 
...
,loca_name_sect_asstwh  varchar (100) 
...
,opeitm_unit_id_case  numeric (38,0)
 ...
 create or replace view r_purrets as select  
  ...
  opeitm.unit_name_prdpurshp  unit_name_prdpurshp ,
  opeitm.unit_code_prdpurshp  unit_code_prdpurshp ,
  .person_name_chrg  person_name_chrg ,
...
purret.id id,
  ...
  .opeitm_esttosch  opeitm_esttosch ,
  chrg.chrg_person_id_chrg  chrg_person_id_chrg ,
  .usrgrp_code_chrg  usrgrp_code_chrg ,
  .usrgrp_name_chrg  usrgrp_name_chrg ,
...
,opeitm_stktaking_proc  varchar (1) 
 ...
 CREATE TABLE sio.sio_r_acpschs (
          sio_id numeric(22,0)  CONSTRAINT SIO_r_acpschs_id_pk PRIMARY KEY           ,sio_user_code numeric(22,0)
          ,sio_Term_id varchar(30)
          ,sio_session_id numeric(22,0)
          ,sio_Command_Response char(1)
          ,sio_session_counter numeric(22,0)
...
prdest.starttime  prdest_starttime,
  prjno.prjno_name_chil  prjno_name_chil 
 from prdests   prdest,
  r_locas  loca_to ,  r_prjnos  prjno ,  r_opeitms  opeitm ,  r_persons  person_upd ,  r_chrgs  chrg 
  where       prdest.locas_id_to = loca_to.id      and prdest.prjnos_id = prjno.id      and prdest.opeitms_id = opeitm.id      and prdest.persons_id_upd = person_upd.id      and prdest.chrgs_id = chrg.id     ;
...
,opeitm_shuffle_loca  varchar (1) 
,opeitm_chkord_proc  numeric (3,0)
,prdest_loca_id_to  numeric (22,0)
,prdest_prjno_id  numeric (22,0)
,prdest_opeitm_id  numeric (22,0)
,prdest_id  numeric (22,0)
,prdest_update_ip  varchar (40) 
,prdest_created_at   timestamp(6) 
,prdest_updated_at   timestamp(6) 
,prdest_person_id_upd  numeric (22,0)
,opeitm_esttosch  numeric (22,0)
,usrgrp_code_chrg  varchar (50) 
,usrgrp_name_chrg  varchar (100) 
,opeitm_rule_price  varchar (1) 
...
purest.crrs_id   purest_crr_id,
  prjno.prjno_name_chil  prjno_name_chil 
 from purests   purest,
  r_prjnos  prjno ,  r_opeitms  opeitm ,  r_persons  person_upd ,  r_locas  loca_to ,  r_chrgs  chrg ,  r_suppliers  supplier ,  r_crrs  crr 
  where       purest.prjnos_id = prjno.id      and purest.opeitms_id = opeitm.id      and purest.persons_id_upd = person_upd.id      and purest.locas_id_to = loca_to.id      and purest.chrgs_id = chrg.id      and purest.suppliers_id = supplier.id      and purest.crrs_id = crr.id     ;
...
,purest_qty  numeric (38,4)
,purest_price  numeric (38,4)
,purest_amt  numeric (38,4)
,purest_sno  varchar (40) 
,purest_duedate   timestamp 
,purest_toduedate   timestamp 
,purest_isudate   timestamp 
...
,itm_code  varchar (50) 
,itm_std  varchar (50) 
,purest_update_ip  varchar (0) 
,purest_created_at   timestamp 
,purest_updated_at   timestamp 
          ,sio_errline varchar(4000)
          ,sio_org_tblname varchar(30)
          ,sio_org_tblid numeric(22,0)
          ,sio_add_time date
...
,nditm_created_at   timestamp(6) 
 ...
 ALTER TABLE prdests ADD CONSTRAINT prdest_persons_id_upd FOREIGN KEY (persons_id_upd)
																		 REFERENCES persons (id);
 ALTER TABLE prdests ADD CONSTRAINT prdest_locas_id_to FOREIGN KEY (locas_id_to)
																		 REFERENCES locas (id);
 ALTER TABLE prdests ADD CONSTRAINT prdest_prjnos_id FOREIGN KEY (prjnos_id)
																		 REFERENCES prjnos (id);
 ALTER TABLE prdests ADD CONSTRAINT prdest_opeitms_id FOREIGN KEY (opeitms_id)
																		 REFERENCES opeitms (id);
 ALTER TABLE prdests ADD CONSTRAINT prdest_chrgs_id FOREIGN KEY (chrgs_id)
																		 REFERENCES chrgs (id);
 ALTER TABLE purests ADD CONSTRAINT purest_suppliers_id FOREIGN KEY (suppliers_id)
																		 REFERENCES suppliers (id);
 ALTER TABLE purests ADD CONSTRAINT purest_persons_id_upd FOREIGN KEY (persons_id_upd)
																		 REFERENCES persons (id);
 ALTER TABLE purests ADD CONSTRAINT purest_locas_id_to FOREIGN KEY (locas_id_to)
																		 REFERENCES locas (id);
 ALTER TABLE purests ADD CONSTRAINT purest_prjnos_id FOREIGN KEY (prjnos_id)
																		 REFERENCES prjnos (id);
 ALTER TABLE purests ADD CONSTRAINT purest_opeitms_id FOREIGN KEY (opeitms_id)
																		 REFERENCES opeitms (id);
 ALTER TABLE purests ADD CONSTRAINT purest_chrgs_id FOREIGN KEY (chrgs_id)
																		 REFERENCES chrgs (id);
 ALTER TABLE purests ADD CONSTRAINT purest_crrs_id FOREIGN KEY (crrs_id)
																		 REFERENCES crrs (id);
