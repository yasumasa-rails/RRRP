 ...
 alter table  mkordopeitms  ADD COLUMN itms_id numeric(38,0);

 ALTER TABLE mkordopeitms ADD CONSTRAINT mkordopeitm_itms_id FOREIGN KEY (itms_id)
																		 REFERENCES itms (id);
...
mkordopeitm.itms_id   mkordopeitm_itm_id,
 ...
 from mkordopeitms   mkordopeitm,
  r_persons  person_upd ,  r_mkords  mkord ,  r_locas  loca ,  r_prjnos  prjno ,  r_shelfnos  shelfno_to ,  r_itms  itm 
  where       mkordopeitm.persons_id_upd = person_upd.id      and mkordopeitm.mkords_id = mkord.id      and mkordopeitm.locas_id = loca.id      and mkordopeitm.prjnos_id = prjno.id      and mkordopeitm.shelfnos_id_to = shelfno_to.id      and mkordopeitm.itms_id = itm.id     ;
