 ...
 alter table  prdacts  ADD COLUMN workplaces_id numeric(22,0);

 alter table prdinsts DROP COLUMN replydate CASCADE;
 ...
 ---- update screenfields set expiredate ='2000/01/01',remark =' 項目　replydateが削除　2020-04-28 21:03:02 +0900' 
 ---- where  pobject_code_sfd = 'replydate'
 alter table prdinsts DROP COLUMN sno_ord CASCADE;
 ...
 --- select * from pobject_code_scr,pobject_code_sfd,
							...
							---	case screenfield_hideflg when 1 then '' else '表示有' end display,
							---   case screenfield_indisp when 1 then '必須' else '' end inquire from r_screenfields 
 ---- where  pobject_code_sfd = 'sno_ord'
 ---- update screenfields set expiredate ='2000/01/01',remark =' 項目　sno_ordが削除　2020-04-28 21:03:02 +0900' 
 ---- where  pobject_code_sfd = 'sno_ord'
 alter table  prdinsts ALTER COLUMN qty  TYPE numeric(22,6);

 alter table  prdinsts ALTER COLUMN qty_case  TYPE numeric(22,0);

 alter table prdinsts  ADD COLUMN sno_prdord varchar(50);

 alter table  prdinsts  ADD COLUMN shelfnos_id_to numeric(38,0);

 alter table prdinsts  ADD COLUMN cno_prdord varchar(50);

 alter table  prdinsts  ADD COLUMN workplaces_id numeric(22,0);

 alter table  prdords ALTER COLUMN qty  TYPE numeric(22,6);

 alter table  prdords ALTER COLUMN qty_case  TYPE numeric(22,0);

 alter table  prdords  ADD COLUMN workplaces_id numeric(22,0);

 ALTER TABLE prdacts ADD CONSTRAINT prdact_workplaces_id FOREIGN KEY (workplaces_id)
																		 REFERENCES workplaces (id);
 ALTER TABLE prdinsts ADD CONSTRAINT prdinst_shelfnos_id_to FOREIGN KEY (shelfnos_id_to)
																		 REFERENCES shelfnos (id);
 ALTER TABLE prdinsts ADD CONSTRAINT prdinst_workplaces_id FOREIGN KEY (workplaces_id)
																		 REFERENCES workplaces (id);
 ALTER TABLE prdords ADD CONSTRAINT prdord_workplaces_id FOREIGN KEY (workplaces_id)
																		 REFERENCES workplaces (id);
