...
billord.locas_id_to   billord_loca_id_to
 from billords   billord,
  r_bills  bill ,  r_itms  itm ,  r_locas  loca_to 
  where       billord.bills_id = bill.id      and billord.itms_id = itm.id      and billord.locas_id_to = loca_to.id     ;
...
payord.payments_id_pay   payord_payment_id_pay,
 ...
 from payords   payord,
  r_itms  itm ,  r_chrgs  chrg ,  r_suppliers  supplier ,  r_crrs  crr_pur ,  r_payments  payment_pay 
  where       payord.itms_id = itm.id      and payord.chrgs_id = chrg.id      and payord.suppliers_id = supplier.id      and payord.crrs_id_pur = crr_pur.id      and payord.payments_id_pay = payment_pay.id     ;
...
mkord.duedate_org  mkord_duedate_org
 from mkords   mkord,
  r_locas  loca_org ,  r_locas  loca_trn ,  r_itms  itm_org ,  r_itms  itm_trn ,  r_chrgs  chrg_trn ,  r_locas  loca_pare ,  r_itms  itm_pare ,  r_locas  loca_to 
  where       mkord.locas_id_org = loca_org.id      and mkord.locas_id_trn = loca_trn.id      and mkord.itms_id_org = itm_org.id      and mkord.itms_id_trn = itm_trn.id      and mkord.chrgs_id_trn = chrg_trn.id      and mkord.locas_id_pare = loca_pare.id      and mkord.itms_id_pare = itm_pare.id      and mkord.locas_id_to = loca_to.id     ;
...
prdord.sno_prdsch  prdord_sno_prdsch
 from prdords   prdord,
  r_prjnos  prjno ,  r_opeitms  opeitm ,  r_chrgs  chrg ,  r_shelfnos  shelfno_to ,  r_workplaces  workplace 
  where       prdord.prjnos_id = prjno.id      and prdord.opeitms_id = opeitm.id      and prdord.chrgs_id = chrg.id      and prdord.shelfnos_id_to = shelfno_to.id      and prdord.workplaces_id = workplace.id     ;
...
purord.crrs_id   purord_crr_id,
  ...
  supplier.crr_name_payment  crr_name_payment 
 from purords   purord,
  r_opeitms  opeitm ,  r_prjnos  prjno ,  r_chrgs  chrg ,  r_suppliers  supplier ,  r_crrs  crr_pur ,  r_shelfnos  shelfno_to ,  r_crrs  crr 
  where       purord.opeitms_id = opeitm.id      and purord.prjnos_id = prjno.id      and purord.chrgs_id = chrg.id      and purord.suppliers_id = supplier.id      and purord.crrs_id_pur = crr_pur.id      and purord.shelfnos_id_to = shelfno_to.id      and purord.crrs_id = crr.id     ;
...
shpord.gno_shpsch  shpord_gno_shpsch
 from shpords   shpord,
  r_locas  loca_to ,  r_itms  itm ,  r_chrgs  chrg ,  r_prjnos  prjno ,  r_transports  transport ,  r_crrs  crr ,  r_shelfnos  shelfno_fm 
  where       shpord.locas_id_to = loca_to.id      and shpord.itms_id = itm.id      and shpord.chrgs_id = chrg.id      and shpord.prjnos_id = prjno.id      and shpord.transports_id = transport.id      and shpord.crrs_id = crr.id      and shpord.shelfnos_id_fm = shelfno_fm.id     ;
...
custord.shelfnos_id_fm   custord_shelfno_id_fm
 from custords   custord,
  r_custs  cust ,  r_prjnos  prjno ,  r_chrgs  chrg_cpo ,  r_custrcvplcs  custrcvplc ,  r_opeitms  opeitm ,  r_shelfnos  shelfno_fm 
  where       custord.custs_id = cust.id      and custord.prjnos_id = prjno.id      and custord.chrgs_id_cpo = chrg_cpo.id      and custord.custrcvplcs_id = custrcvplc.id      and custord.opeitms_id = opeitm.id      and custord.shelfnos_id_fm = shelfno_fm.id     ;
