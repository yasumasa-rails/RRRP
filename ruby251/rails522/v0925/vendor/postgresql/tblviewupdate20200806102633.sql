 ...
 alter table mkords DROP COLUMN fullord_pare CASCADE;
 ...
 ---- where  pobject_code_sfd = 'fullord_pare'
 ---- update screenfields set expiredate ='2000/01/01',remark =' 項目　fullord_pareが削除　2020-08-06 10:26:31 +0900' 
 ---- where  pobject_code_sfd = 'fullord_pare'
 alter table mkords DROP COLUMN locas_id_to CASCADE;
 ...
 ---- update screenfields set expiredate ='2000/01/01',remark =' 項目　locas_id_toが削除　2020-08-06 10:26:31 +0900' 
 ---- where  pobject_code_sfd = 'locas_id_to'
 alter table mkords  ADD COLUMN sno_trn varchar(50);

 alter table mkords  ADD COLUMN gno_trn varchar(50);
...
mkord.paretblname  mkord_paretblname
 from mkords   mkord,
  r_persons  person_upd ,  r_chrgs  chrg_trn ,  r_itms  itm_org ,  r_locas  loca_org ,  r_itms  itm_pare ,  r_locas  loca_pare ,  r_itms  itm_trn ,  r_locas  loca_trn ,  r_locas  loca_to 
  where       mkord.persons_id_upd = person_upd.id      and mkord.chrgs_id_trn = chrg_trn.id      and mkord.itms_id_org = itm_org.id      and mkord.locas_id_org = loca_org.id      and mkord.itms_id_pare = itm_pare.id      and mkord.locas_id_pare = loca_pare.id      and mkord.itms_id_trn = itm_trn.id      and mkord.locas_id_trn = loca_trn.id      and mkord.locas_id_to = loca_to.id     ;
