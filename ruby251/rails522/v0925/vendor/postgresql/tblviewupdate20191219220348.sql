 ...
 alter table  shpschs  ADD COLUMN crrs_id numeric(22,0);

 ALTER TABLE shpschs ADD CONSTRAINT shpsch_crrs_id FOREIGN KEY (crrs_id)
																		 REFERENCES crrs (id);
...
shpsch.trngantts_id_shp   shpsch_trngantt_id_shp,
  ...
  chrg.scrlv_code  scrlv_code ,
  chrg.scrlv_level1  scrlv_level1 ,
  chrg.person_name  person_name ,
  chrg.person_email  person_email ,
  chrg.person_code  person_code ,
  chrg.usrgrp_code  usrgrp_code ,
  chrg.usrgrp_name  usrgrp_name ,
  chrg.person_email_chrg  person_email_chrg 
 from shpschs   shpsch,
  r_crrs  crr ,  r_persons  person_upd ,  r_prjnos  prjno ,  r_chrgs  chrg ,  r_transports  transport ,  r_trngantts  trngantt_shp 
  where       shpsch.crrs_id = crr.id      and shpsch.persons_id_upd = person_upd.id      and shpsch.prjnos_id = prjno.id      and shpsch.chrgs_id = chrg.id      and shpsch.transports_id = transport.id      and shpsch.trngantts_id_shp = trngantt_shp.id     ;
