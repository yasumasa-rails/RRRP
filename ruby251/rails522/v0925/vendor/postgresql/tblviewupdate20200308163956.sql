 ...
 alter table shpschs DROP COLUMN starttime CASCADE;
 ...
 ---- where  pobject_code_sfd = 'starttime'
 ---- update screenfields set expiredate ='2000/01/01',remark =' 項目　starttimeが削除　2020-03-08 16:39:53 +0900' 
 ---- where  pobject_code_sfd = 'starttime'
 alter table shpschs DROP COLUMN duedate CASCADE;
 ...
 --- select * from pobject_code_scr,pobject_code_sfd,
							...
							---	case screenfield_hideflg when 1 then '' else '表示有' end display,
							---   case screenfield_indisp when 1 then '必須' else '' end inquire from r_screenfields 
 ---- where  pobject_code_sfd = 'duedate'
 ---- update screenfields set expiredate ='2000/01/01',remark =' 項目　duedateが削除　2020-03-08 16:39:53 +0900' 
 ---- where  pobject_code_sfd = 'duedate'
 alter table shpschs DROP COLUMN trngantts_id_shp CASCADE;
 ...
 --- select * from pobject_code_scr,pobject_code_sfd,
							...
							---	case screenfield_hideflg when 1 then '' else '表示有' end display,
							---   case screenfield_indisp when 1 then '必須' else '' end inquire from r_screenfields 
 ---- where  pobject_code_sfd = 'trngantts_id_shp'
 ---- update screenfields set expiredate ='2000/01/01',remark =' 項目　trngantts_id_shpが削除　2020-03-08 16:39:53 +0900' 
 ---- where  pobject_code_sfd = 'trngantts_id_shp'
 alter table  shpschs  ADD COLUMN itms_id numeric(38,0);

 alter table  shpschs  ADD COLUMN locas_id_to numeric(38,0);
 ...
 alter table shpschs  ADD COLUMN paretblname varchar(30);

 alter table  shpschs  ADD COLUMN paretblid numeric(38,0);

 alter table  shpschs  ADD COLUMN shelfnos_id_fm numeric(22,0);

 ALTER TABLE shpschs ADD CONSTRAINT shpsch_itms_id FOREIGN KEY (itms_id)
																		 REFERENCES itms (id);
 ALTER TABLE shpschs ADD CONSTRAINT shpsch_locas_id_to FOREIGN KEY (locas_id_to)
																		 REFERENCES locas (id);
 ALTER TABLE shpschs ADD CONSTRAINT shpsch_shelfnos_id_fm FOREIGN KEY (shelfnos_id_fm)
