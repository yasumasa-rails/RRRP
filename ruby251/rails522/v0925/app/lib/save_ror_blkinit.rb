...
module RorBlkinit  
	def proc_blkgetpobj_init code,ptype,current_user
		##sub_blkgetpobj code,ptype
	##end
    ##def sub_blkgetpobj code,ptype    ###
        fstrsqly = ""
        ##grp_code =  sub_blkget_grpcode 
		if RorBlkctl.grp_code(current_user) == "batch"
            oragname = code
		end
		if code =~ /_id/ or   code == "id" 
			...
			orgname = ""      
			basesql = "select pobjgrp_name from R_POBJGRPS where POBJGRP_EXPIREDATE > current_date AND USRGRP_CODE = '#{RorBlkctl.(current_user)}' and   "
    ...
    def sub_blkget_code_fm_name_init name,ptype    ###
         ...
         if name =~ /_id/ or   name == "id" then
            ...
            orgcode = ""      
            basesql = "select POBJECT_CODE  from R_POBJGRPS where POBJGRP_EXPIREDATE > current_date AND USERGROUP_CODE = '#{RorBlkctl.grp_code(current_user)}' and   "
