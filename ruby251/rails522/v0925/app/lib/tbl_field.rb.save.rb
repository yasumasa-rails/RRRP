...
extend self
	...
	def add_screenfield_record screens_id,pobjects_id_sfd,field
		if @save_user !=  @current_api_user or @params[:screenCode] ==   @params_screenCode
			...
			command_r["screenfield_selection"] != "0"
		...
		ActiveRecord::Base.connection.select_all(strsql).each do |rec|
			if rec
				if rec["screenfield_crtfield"] and rec["pobject_code_sfd"].split("_")[0] != tbl.chop
					chktbl_delm = rec["screenfield_crtfield"].split("_",2)
					chktbl = chktbl_delm[0]
					delm = chktbl_delm[1]
					strsql = %Q%select 1 from r_tblfields where pobject_code_tbl = '#{tbl}' 
												and pobject_code_fld = '#{chktbl}s_id}' %
					if delm
						field = rec["pobject_code_sfd"].split(/_#{delm}$/)[0]
					else
						field = rec["pobject_code_sfd"]
					end		
					if chkfields.empty? or chkfields["r_#{chktbl}s"].nil?
						strsql = "select	table_name,column_name from 	information_schema.columns 
											where 	table_catalog='#{ActiveRecord::Base.configurations["development"]["database"]}' 
											and table_name='r_#{chktbl}s' "
						chks = ActiveRecord::Base.connection.select_all(strsql)
						chks.each do |chkrec|
							if chkfields[chkrec["table_name"]].nil?
								chkfields[chkrec["table_name"]] = {}
							end
							if chkfields[chkrec["table_name"]][chkrec["column_name"]].nil? 
								chkfields[chkrec["table_name"]][chkrec["column_name"]] = 1
							else
								chkfields[chkrec["table_name"]][chkrec["column_name"]] += 1
							end
						end
					end
					if chkfields["r_#{chktbl}s"]
						if  chkfields["r_#{chktbl}s"][field]
							if  chkfields["r_#{chktbl}s"][field] == 1
							else
								@messages << "<p>step 1: view filed r_#{chktbl}s.#{field} duplicate </p>"
							end
						else
							@messages << "<p> step 0 : view filed r_#{chktbl}s.#{field} not exists  </p>"
						end
					else
						@messages << "<p> view r_#{chktbl} not exists  1</p>"
					end	
				else
					next if rec["pobject_code_sfd"] == "id"
					tbln = rec["pobject_code_sfd"].split("_")[0]+ "s"
					columnn = rec["pobject_code_sfd"].split("_",2)[1].gsub("_id","s_id")
					if chkfields.empty? or chkfields[tbln].nil?
						strsql = "select	table_name,column_name from 	information_schema.columns 
												where 	table_catalog='#{ActiveRecord::Base.configurations["development"]["database"]}' 
												and table_name='#{tbln}'	and column_name = '#{columnn}' "
						chks = ActiveRecord::Base.connection.select_all(strsql)
						chks.each do |chkrec|
							if chkfields[chkrec["table_name"]].nil?
								chkfields[chkrec["table_name"]] = {}
